...
def moveOn(dir,board):
    ...
    if dir == 'up':
        if board[x_head-1][y_head] == 3:
            board = getFruit(board,x_head-1,y_head)
    ...
    if dir == 'left':
        if board[x_head][y_head-1] == 3:
            board = getFruit(board,x_head,y_head-1)
